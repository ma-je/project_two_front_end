...
var blogApi = __webpack_require__(11);
var blogUi = __webpack_require__(12);
...
var onSignup = function onSignup(event) {
  ...
  if (data.credentials.password === data.credentials.password_confirmation) {
    blogApi.addUser(data).then(blogUi.onSignupSuccess) // if works
...
var onSignIn = function onSignIn(event) {
  ...
  blogApi.logInUser(data).then(blogUi.onSignInSuccess).catch(blogUi.signInFailure); // server response
};
...
var onSignOut = function onSignOut(event) {
  ...
  blogApi.logOut(data).then(blogUi.onSignOutSuccess).catch(blogUi.onError);
};
...
var onPasswordReset = function onPasswordReset(event) {
  ...
  blogApi.resetPassword(data).then(blogUi.resetSuccess).catch(blogUi.onError);
};
// new posts

var onPostSuccess = function onPostSuccess(event) {
  event.preventDefault();
  var data = getFormFields(this);
  // console.log(data)
  ...
  blogApi.createPost(data).then(blogUi.onPostSuccess).catch(blogUi.onError);
};
// comments
var onCommentSuccess = function onCommentSuccess(event) {
  event.preventDefault();
  var data = getFormFields(this);
  // console.log(data)
  // console.log('commented successfully')
  blogApi.newComment(data).then(blogUi.commentSuccess).catch(blogUi.joinError);
};
// get post
var onGetPostSuccess = function onGetPostSuccess(event) {
  event.preventDefault();
  var data = app.user;
  blogApi.getPost(data).then(blogUi.getPostsSuccess).catch(blogUi.onError);
};
// editing post
var onEditPostSuccess = function onEditPostSuccess(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  console.log(data);
  blogApi.editPost(data).then(blogUi.editPostSuccess).catch(blogUi.onError);
};
// delete posts

var onDeletePostSuccess = function onDeletePostSuccess(event) {
  blogApi.deletePost(event.target.dataset.id).then(blogUi.DeletPostSuccess).catch(blogUi.onError);
};
...
module.exports = {
  ...
  onGetPostSuccess: onGetPostSuccess,
...
var blogEvents = __webpack_require__(4);
...
// delete a post
var deletePost = function deletePost(deleteId) {
  return $.ajax({
...
var resetFailure = function resetFailure(error) {
  console.log(error);
  // console.log('password changed successfully')
};
// creating posts successfully
var onPostSucess = function onPostSucess(data) {
  app.data = data.post;
  console.log(data);
...
// new commentSuccess
var onCommentSucess = function onCommentSucess(data) {
  console.log('successfully submitted');
};

//  get posts
var loopPosts = function loopPosts(data) {
  console.log(data);
  console.log(data.posts.length);
  $('#posts-section').empty();
  for (var i = 0; i < data.posts.length; i++) {
    $('#posts-section').append('<div> ' + '<h2>' + data.posts[i].title + '</h2>' + '<p>' + data.posts[i].content + '</p>' + '<p class="number">' + data.posts[i].id + '<p>' + '<button class="delete-post" data-id="' + data.posts[i].id + '" id="' + data.posts[i].id + '" type="button" class="btn">Delete</button>' + '<button class="edit-post" id="' + data.posts[i].id + '" type="button" class="btn">Edit</button>' + '</div>');
  }
  var admin = app.user.admin;
  if (admin === true) {
    $('#new-post').show();
...
module.exports = {
  ...
  onPostSucess: onPostSucess,
  getPostsSuccess: getPostsSuccess,
  onCommentSucess: onCommentSucess,
